...
<section class="publications">
  ...
  <div style="margin: 2rem 0 3rem 0; padding: 1.5rem; background: #2d2d2d; border-radius: 8px;">
    ...
    <div style="display: flex; flex-direction: column; align-items: center;">
      ...
      <div style="display: flex; align-items: flex-end; gap: 0.5rem; justify-content: center; flex-wrap: wrap; width: 100%; margin-bottom: 0.5rem; min-height: 220px;">
          ...
          {% for year in publications_by_year %}
            {% if year[0] == year_str %}
              ...
              <div style="flex: 1; min-width: 50px; max-width: 80px; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: flex-end;">
                <div style="background: linear-gradient(to top, #4a90e2 0%, #357abd 100%); height: {{ bar_height }}px; width: 100%; border-radius: 4px 4px 0 0; position: relative; transition: opacity 0.3s; {% if count == 0 %}opacity: 0.3;{% endif %}" title="{{ count }} publication{% if count != 1 %}s{% endif %} in {{ year[0] }}">
                  {% if count > 0 %}
      ...
      <!-- X-axis labels (years) -->
      <div style="display: flex; gap: 0.5rem; justify-content: center; flex-wrap: wrap; width: 100%; border-top: 1px solid #444; padding-top: 0.5rem;">
        {% for year_num in sorted_years %}
          {% assign year_str = year_num | append: '' %}
